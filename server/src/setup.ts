...
export function setup() {
    ...
    let gh_client_id = env.CCN_GITHUB_CLIENT_ID || env.GITHUB_CLIENT_ID;
    let gh_client_secret = env.CCN_GITHUB_CLIENT_SECRET || env.GITHUB_CLIENT_SECRET;
    let jwt_secret = env.JWT_SECRET;

    if (!gh_client_id || !gh_client_secret) {
        throw new Error('Please set CCN_GITHUB_CLIENT_ID and CCN_GITHUB_CLIENT_SECRET or GITHUB_CLIENT_ID and GITHUB_CLIENT_SECRET');
    }
    if (!jwt_secret) {
